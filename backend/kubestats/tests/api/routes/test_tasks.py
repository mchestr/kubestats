...
def test_trigger_log_task_success(
    ...
    # Test data
    task_data = {"message": "Test log message", "log_level": "INFO", "duration": 5}
    ...
    mock_task_delay.assert_called_once_with(
        message="Test log message", log_level="INFO", duration=5
    )
...
def test_trigger_task_unauthorized(client: TestClient) -> None:
    ...
    task_data = {"message": "Unauthorized test", "log_level": "INFO", "duration": 3}
...
def test_get_worker_stats_success(
    ...
    mock_get_worker_stats.assert_called_once_with(
        db=mock_get_worker_stats.call_args[1]["db"], worker_id=worker_id
    )
...
def test_task_routes_require_superuser(
    ...
    for method, endpoint, data in endpoints:
        if method == "POST":
            response = client.post(
                endpoint, headers=normal_user_token_headers, json=data
            )
        else:
...
def test_trigger_task_celery_error(
    ...
    task_data = {"message": "Test message", "log_level": "INFO", "duration": 3}
...
def test_task_trigger_request_validation() -> None:
    ...
    from kubestats.api.routes.tasks import TaskTriggerRequest

    # Valid request
    valid_request = TaskTriggerRequest(
        message="Test message", log_level="INFO", duration=5
    )
...
def test_task_response_model() -> None:
    ...
    from kubestats.api.routes.tasks import TaskResponse

    response = TaskResponse(
        task_id="test-123", status="PENDING", message="Task started"
    )
...
def test_task_status_response_model() -> None:
    ...
    from kubestats.api.routes.tasks import TaskStatusResponse
    ...
    # With minimal fields
    minimal_response = TaskStatusResponse(task_id="test-789", status="PENDING")
...
def test_task_status_response_exception_serialization() -> None:
    ...
    response = TaskStatusResponse(
        task_id="exception-test", status="FAILURE", result=exception
    )
...
def test_worker_stats_response_model() -> None:
    ...
    from kubestats.api.routes.tasks import WorkerStatsResponse
    ...
    minimal_response = WorkerStatsResponse(
        worker_id="worker-2", worker_name="celery@worker-2", status="OFFLINE"
    )
