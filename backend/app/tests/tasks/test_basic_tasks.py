...
def test_create_log_entry_exception_handling(_mock_sleep) -> None:
    ...
    with patch('app.tasks.basic_tasks.create_log_entry'):
        ...
        mock_task_instance.update_state = Mock(side_effect=Exception("Update state error"))
...
@patch('app.tasks.basic_tasks.datetime')
def test_system_health_check_success(mock_datetime, mock_memory, mock_cpu, mock_redis) -> None:
...
@patch('psutil.cpu_percent')
@patch('app.tasks.basic_tasks.datetime')
def test_system_health_check_psutil_failure(mock_datetime, mock_cpu, mock_redis) -> None:
